<x-app-layout>
  ...
  <style>
    ...
    .truncate-email{display:inline-block;max-width:260px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
  </style>
  ...
  {{-- Script: acción y conteo para la asignación masiva --}}
  @push('scripts')
  <script>
    (function () {
      const bulkForm   = document.getElementById('bulk-form');
      const bulkGrupo  = document.getElementById('bulk-grupo');
      const bulkBtn    = document.getElementById('bulk-assign-btn');
      const bulkCount  = document.getElementById('bulk-count');
      const checks     = Array.from(document.querySelectorAll('.bulk-chk'));

      function updateCount() {
        const n = checks.filter(c => c.checked).length;
        bulkCount.textContent = n + ' seleccionados';
        bulkBtn.disabled = !(n > 0 && bulkGrupo.value);
      }

      function updateAction() {
        const id = bulkGrupo.value;
        // Importante: la ruta existe dentro del prefijo /admin en web.php
        bulkForm.action = id ? (`{{ url('admin/grupos') }}/${id}/asignar`) : '';
        updateCount();
      }

      checks.forEach(c => c.addEventListener('change', updateCount));
      bulkGrupo.addEventListener('change', updateAction);
